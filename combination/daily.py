...
def sentimentVariables(df):
    print(df)
    for value in df.index.values:
        try:
            #get the right output file
            tempdf = pd.read_csv("sentimentOutputs/" + str(value).replace("T","H",1).split(':')[0] + ".csv")
            df.loc[value,["day_sentiment"]] = tempdf.iloc[0]["day_sentiment"]
            df.loc[value,["highest_score"]] = tempdf.iloc[0]["highest_score"]
            df.loc[value,["lowest_score"]] = tempdf.iloc[0]["lowest_score"]
            df.loc[value,["number_of_posts"]] = tempdf.iloc[0]["number_of_posts"]
            df.loc[value,["standard_deviation"]] = tempdf.iloc[0]["standard_deviation"]
            df.loc[value,["skewness"]] = tempdf.iloc[0]["skewness"]
            df.loc[value,["kurtosis"]] = tempdf.iloc[0]["kurtosis"]

        except:
            print(str(value).replace("T","H",1).split(':')[0], " niet gevonden!")
...
def main(start_date, end_date):
    ...
    df = yf.download('BTC-USD', start-dt.timedelta(days=3), end+dt.timedelta(days=32), interval="1h")
    technicalIndicators(df)
    
    df = transformDataFrame(df)
    print("test1")
    sentimentVariables(df)  
